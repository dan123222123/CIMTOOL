...
contour = Numerics.Contour.Ellipse(-0.3+62i,0.05,0.75,5e2);
tc = Numerics.Contour.Ellipse(-0.3+62i,0.05,0.75,5e2);
...
%% contour conga
g = @(x) x - 200*1i*x; gx = linspace(-0.3,0,5); gls = g(gx);
...
wobj = 'cc_iss.gif'; delete(wobj);
for i=1:length(gls)
    % test if there are ew inside the contour
    tc.gamma = gls(i); nec = length(ewref(tc.inside(ewref)));
    % compute and plot realized tf response
    if nec ~= 0
        CIM.SampleData.Contour.gamma = gls(i); CIM.RealizationData.m = nec;
        CIM.compute(); [~,V2,W2,M21,M22] = CIM.ResultData.rtf(nec); Hrmpl = @(z) V2*((M21-z*M22)\W2);
        plot_cim_response(f,CIM,H,Hrmpl,w);
    else
        plot_cim_response(f,CIM,H,Hrmpl,w); sgtitle(f,fprintf("at i=%d, nec was %d\n",i,nec))
    end
    exportgraphics(gcf,wobj,'Append',true,'Resolution',80)
end

function plot_cim_response(f,CIM,H,Hr,w)
    drawnow nocallbacks;
