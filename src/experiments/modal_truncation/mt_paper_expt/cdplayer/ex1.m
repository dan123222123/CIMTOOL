...
NL = 100; x = linspace(alpha,10,NL); y = linspace(gamma,0,NL); z = linspace(beta,1e4,NL);
for i = 1:NL
    CIMMPL.SampleData.Contour.beta = z(i);
    CIMMPL.SampleData.Contour.gamma = y(i);
    CIMMPL.SampleData.Contour.alpha = x(i);
    %
    CIMMPL.RealizationData.InterpolationData = vertshiftline(NN,lep,ss*(CIMMPL.SampleData.Contour.gamma+CIMMPL.SampleData.Contour.alpha));
    %
    nec = length(ewref(CIMMPL.SampleData.Contour.inside(ewref)));
    %
